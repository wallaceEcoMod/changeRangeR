...
```{r}
p <-raster("inst/extdata/DemoData/SDM/olinguito/Forest_suitable_projected1.tif")
...
```{r}
p <-raster("inst/extdata/DemoData/SDM/olinguito/Climatically_suitable_projected1.tif")
...
```{r}
p <- raster("inst/extdata/DemoData/SDM/olinguito/olinguitoSDM.tif")
...
```{r, warning = F}
r <-raster("inst/extdata/DemoData/SDM/olinguito/Forest_suitable_projected1.tif")
...
```{r}
# Load shapefile
PA <- st_read("C:/Users/pgalante/OneDrive - AMNH/SDM_special_Issue/PAs/VN_Protected_Areas", "VN_NRs")
# load raster
r <- stack(list.files(path = "C:/Users/pgalante/OneDrive - AMNH/SDM_special_Issue/SpeciesRasters/Francois", pattern = "\\.tif$", full.names = T))
# Assume PA's will not change, so make list of current protectes areas
futures <- list(PA, PA)
# create list of rasters for example
r <- raster::unstack(r[[c(1,4)]])
...
plot(y = SDM.time, x = years, main = "SDM area change", ylab = "area (square m)")
lines(y = SDM.time, x = years)
```
