...
vignette: |
  %\VignetteIndexEntry{changeRanger Single Species Metrics} %\VignetteEngine{knitr::rmarkdown} %\VignetteEncoding{UTF-8}
---
...
```{r}
p <-raster(paste0(system.file(package="changeRangeR"), "/extdata/DemoData/SDM/olinguito/Forest_suitable_projected1.tif"))
...
```{r}
p <- raster(paste0(system.file(package="changeRangeR"), "/extdata/DemoData/SDM/olinguito/Climatically_suitable_projected1.tif"))
...
In changeRanger, users can calculate AOO either 1) with occurrence points, 2) from the pre-masked thresholded SDM, and 3) from the masked thresholded SDM. It is suggested that users reproject ranges to UTM or another equal area projection for more accurate area-based calculations.
...
```{r}
p <- raster(paste0(system.file(package="changeRangeR"), "/extdata/DemoData/SDM/olinguito/olinguitoSDM.tif"))
xy <- read.csv(paste0(system.file(package="changeRangeR"), "/extdata/DemoData/locs/10KM_thin_2017.csv"))
ch.orig <- mcp(xy[,2:3])
thr <- 0.3380209
SDMeoo <- mcpSDM(p = p, xy = xy[,2:3], ch.orig = ch.orig, thr = thr)
# Check the output
SDMeoo
```
...
```{r}
# Load shapefile
PA <- st_read(dsn = paste0(system.file(package="changeRangeR"), "/extdata/DemoData/shapefiles/vn"), "VN_NRs")
# load raster
...
```{r}
SDM <- raster::raster(paste0(system.file(package="changeRangeR"), "/extdata/DemoData/SDM/olinguito/Climatically_suitable_projected1.tif"))
...
SDM.time <- envChange(rStack = rStack, SDM = SDM, threshold = threshold)

years <- c("2005", "2006", "2008", "2009", "2010")
