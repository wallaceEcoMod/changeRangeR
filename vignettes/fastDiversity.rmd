...
summaryBaseDir='/Volumes/cm2/changeRangerDemos/trees190_test4'
...
TODO: store all the collapsing info in the index tabels, or optionally in separate attribute tabels. 
...
saveRDS(newSpInd,file=paste0(sumDirs$sumBaseDir,'/speciesIndexTable.rds'))
```
...
```
<!-- CAN DO the next step with sparse count matrices. Todo.
Note that there's a second way to do the same operation as above, which uses species attributes internally in the function without making the cell by genus matrices. This way is less flexible because you can only do one calculation (richness), and is a little slower, but saves you from making extra matrices if your only interest is genus (or some other factor) richness. -->

You might also want to look at richness within each genus separately.

```{r}
gen=readRDS(paste0(sumDirs$sumBaseDir, '/speciesIndexTable.rds'))
genusSpAttr2 = gen %>% select(species) %>%
  separate(species,c("genus",NA),sep='_')  %>%
  fastDummies::dummy_cols()  %>%  select(-genus) %>%
  bind_cols(gen %>% select(species,index))
# not great that you have to subset the spIndex table for this to work, since that may be confusing
richGen2=lapply(allScen,function(scn){
	r=richnessFromCBS(cbsDir=sumDirs$cbsDir,scenario=scn,
	                  env=envGrid,mc.cores=mc.cores,
	                  attrTable=genusSpAttr2,outDir=sumDirs$richByGenusDir)
	names(r)=paste0(names(r),'_',scn)
	fdMapPlot(r,plotFile=paste0(sumDirs$figDir,'/RichnessByGenus_',scn,'.pdf'), 
	          shp=world.shp2, legend.args=list(text='# species',line=2,side=4))
	r
...
# same as last plots
fdMapPlot(richGen2[[1]],shp=world.shp2,legend.args=list(text='Species Richness')) 
fdMapPlot(richGen2[[2]],shp=world.shp2,legend.args=list(text='Species Richness')) 
```
...
As an example of (2) we find the species richness in each ecoregion. This represents a special case because we only need a binary indicator of whether each species occurs in each ecoregion.
Fortunately, this is is straighforward to implement, because we can generate these from the CBS matrices by collapsing the rows that occur in the same ecoregion, thus generating an ecoregion by species matrix. With this, we can use any of the functions developed above on the ecoregion by species matrices to make ecoregion-specific diversity metrics. 
