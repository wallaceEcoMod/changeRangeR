...
<!-- There might be a moment when we might want to use rasters of different characteristics as the one where we created the database, in that case, we might need to generate dictionaries between the cellIDs of two rasters.  -->

<!-- ```{r} -->
<!-- Dictionary <- function(x, y){ -->
<!--   Coords <- as.data.frame(raster::xyFromCell(x, 1:ncell(x))) -->
<!--   Coords$CellID <- 1:ncell(x) -->
<!--   Coords$ID2 <- raster::cellFromXY(y, as.matrix(Coords[,1:2])) -->
<!--   Coords <- Coords[!is.na(Coords$ID2),] -->
<!--   Coords <- Coords[!is.na(Coords$CellID),] -->
<!--   return(Coords) -->
<!-- } -->
<!-- ``` -->


<!-- This function will create it as long as the coordinates of both rasters are compatible, here is an example use: -->

<!-- ```{r} -->
<!-- Env <- raster("envGrid.tif") -->
<!-- richness100 <- raster("richness100km.tif") -->

<!-- Dict1 <- Dictionary(x = Env, y = richness100) -->
<!-- Dict2 <- Dictionary(x = richness100, y = Env) -->
<!-- ``` -->
