...
summaryBaseDir='/Volumes/cm2/changeRangerDemos/trees190_test'
...
#mydir='/Volumes/cm2/NatureMap/Plants_posthoc_corrected'

# shapefiles for plotting. This one comes preinstallted
#world.shp=readOGR(system.file("extdata/TM_WORLD_BORDERS_SIMPL-0.3/TM_WORLD_BORDERS_SIMPL-0.3.shp",package='changeRanger'),'TM_WORLD_BORDERS_SIMPL-0.3',verbose=F)
...
```{r}
# NOT RUN YET; WAITING FOR RAW DATA
for (scn in allScen){
...
# 	allSpeciesMaps=data.frame(rasterFiles=c( list.files(paste0(mydir,'/BIEN_PLANTAE_POINTS_10km'),recursive=T, full.names=T), stringsAsFactors=F)
# 	allSpeciesMaps$sp.names=file_path_sans_ext(basename(allSpeciesMaps$rasterFiles))	
# 	setupSparseSpeciesMatrixStuff(sumDirs,
# 		scenario='Present',
# 		env=envGrid, 
# 		allSpeciesMaps,nCellChunks=100,mc.cores=mc.cores,reprojectToEnv=F, 
# 		myTempPath='/Volumes/cm2/TempFiles',overwrite=F)
}
# temp solution
...
ecoAus=raster(paste0(sumDirs$miscDir,'/aus.tif'))			
...
library(ape)
library(tictoc)
phyloTree=read.tree(paste0(sumDirs$sumBaseDir,'/Misc/selecPhyTree.tree'))
sp.ind=readRDS(paste0(sumDirs$sumBaseDir,'/speciesIndexTable.rds'))
...
#new potential developments (do it for every scenario automatically?)
#new potential developments (write a Raster by default?)
# todo: print the scen internally
# BROKE
# pd=lapply(allScen, function(scn){
#   metricFromCBS(cbsDir=sumDirs$cbsDir,scenario = scn,env = env ,
...
#   	#SPECIFY other arguments of the function
...
#the user can explores this function in more depth
...
#note that the function is aware that unique communities have been computed
# 
# BROKE
